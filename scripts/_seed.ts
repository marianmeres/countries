/**
 * Canonical membership seed for `@marianmeres/countries`.
 *
 * These 234 entries are the authoritative source of truth for `iso`, `name`,
 * `dialCode` and `flag`. They were ported verbatim from the original list that
 * lived in `@marianmeres/stuic` (phone-number input component), so preserving
 * this exact set keeps downstream stuic behavior identical.
 *
 * `scripts/gen-data.ts` reads this seed and merges in the enrichment fields
 * (`iso3`, `numeric`, `continent`, `currency`, `capital`, `nativeName`) from
 * permissively-licensed sources. The enrichment step may only ADD fields — it
 * must never overwrite anything declared here.
 *
 * To add/remove a country, edit this file and re-run `deno task gen`.
 */
