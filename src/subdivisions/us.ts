...
// A point-in-time snapshot of ISO 3166-2:US — a convenience, not an authority.

import type { Subdivision } from "../types.ts";

/**
 * ISO 3166-2:US subdivisions — 50 states + 1 district (DC) + 6 outlying areas
 * (AS, GU, MP, PR, UM, VI) — alphabetical by English name. Side-effect-free
 * data module (tree-shakeable).
 *
 * `code` is the ISO 3166-2 suffix (= USPS abbreviation): `"MI"` -> `"US-MI"`.
 * Want states only? Filter by `category` ("state" | "district" | "outlying area").
 * USPS military "states" (AA/AE/AP) are not ISO subdivisions and are not listed.
 */
// deno-fmt-ignore
// prettier-ignore
const US: Subdivision[] = [
	...
	{ code: "VI", name: "U.S. Virgin Islands", category: "outlying area" },
