...
└── build-npm.ts      # NPM build (entry points: mod, search, timezones, locales/sk)
```
...
3. **Locale loaders must use static string literals.** In `locale.ts`, each
   `BUILTIN_LOCALE_LOADERS` value MUST be `() => import("./locales/<literal>.ts")`.
   A dynamic, template-literal `import()` of a computed path silently breaks the
   npm build and tree-shaking. (LOAD-BEARING — see the comment in `locale.ts`.)
...
- [ ] Run `deno task test` (32 tests; includes the stuic coupling check).
